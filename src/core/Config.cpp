...
{
    return mFileManager.readFile(SPIFFS, authenticationPath);
}

String Config::readPaymentInfo()
{
    return mFileManager.readFile(SPIFFS, paymentInfoPath);
}

String Config::readBoxId()
{
    return mFileManager.readFile(SPIFFS, boxIdPath);
}

void Config::writeWiFiConfig(const char *message)
{
    mFileManager.writeFile(SPIFFS, wifiConfigPath, message);
};

void Config::writeAuthentication(const char *message)
{
    mFileManager.writeFile(SPIFFS, authenticationPath, message);
};

void Config::writePaymentInfo(const char *message)
{
    mFileManager.writeFile(SPIFFS, paymentInfoPath, message);
};

void Config::writeBoxId(const char *message)
{
    mFileManager.writeFile(SPIFFS, boxIdPath, message);
};

String Config::readQrCertificate()
{
    return mFileManager.readFile(SPIFFS, qrCertificatePath);
}
void Config::writeQrCertificate(const char *data)
{
    mFileManager.writeFile(SPIFFS, qrCertificatePath, data);
}